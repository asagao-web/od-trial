...
# class Detect(Function): # maya_ for pytorch 1.4 below
class Detect(): # maya_ for pytorch 1.5 above
    ...
    # def forward(self, loc_data, conf_data, dbox_list): # maya_ for pytorch 1.4 below
    def __call__(self, loc_data, conf_data, dbox_list): # maya_ for pytorch 1.5 above
        """
        順伝搬の計算を実行する。
...
class SSD(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.phase == "inference":  # 推論時
            ...
            # maya_ for pytorch 1.4 below
            # return self.detect(output[0], output[1], output[2])

            # maya_ for pytorch 1.5 above
            with torch.no_grad():
                return self.detect(output[0], output[1], output[2])
